...
def describe_attractor(result, dt, runner, print_results):
    """
...
def equivalent_attractors(a, b):
    ...
    if isinstance(a, dict):
        ...
        if diff_norm > 0.01 * len(a):
            return False
        elif diff_norm > 0.0001 * len(a):
            return diff_norm < min(np.linalg.norm(a), np.linalg.norm(b)) * 0.3
        else:
            return True
...
def findattractors(runner, ic_sets, time, dt, print_warnings=False):
    ...
    for ii, ini_comb in enumerate(ic_sets):
        ...
        attractor = describe_attractor(sim_points, dt, runner, print_warnings)
