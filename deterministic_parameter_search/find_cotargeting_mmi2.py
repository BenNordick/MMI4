...
runner = te.loada('''
// ZEB1 with 2 miR-200 binding sites
JRZ: -> R_ZEB; k_ZEB - R_ZEB
...
// CDH1 transcriptionally repressed by ZEB1 and SNAIL
JRC: -> R_CDH; k_CDH * (f_CDH_cons + f_CDH_ZEB / (1 + (X_ZEB / K_CDH_ZEB)^n_CDH_ZEB) + f_CDH_SNAI / (1 + (X_SNAI / K_CDH_SNAI)^n_CDH_SNAI)) - R_CDH
...
k_200 = 0.1; k_34 = 0.1
k_CDH = 1
f_CDH_ZEB = 0.25; f_CDH_SNAI = 0.7; f_CDH_cons := 1 - f_CDH_ZEB - f_CDH_SNAI
K_CDH_ZEB = 0.1; n_CDH_ZEB = 1; K_CDH_SNAI = 0.1; n_CDH_SNAI = 1
...
results = [{'attractors': 4}]
while len(results) < args.count:
    pset = {}
    pset['k_CDH'] = 10 ** np.random.uniform(-0.5, 0.5)
    for mi in ['200', '34']:
        pset[f'k_{mi}'] = 10 ** np.random.uniform(-1.5, -0.2)
    for target in ['ZEB', 'SNAI']:
        pset[f'l_{target}'] = 10 ** np.random.uniform(0, 1)
        pset[f'k_{target}'] = 10 ** np.random.uniform(-0.5, 0.5)
        pset[f'K_CDH_{target}'] = 10 ** np.random.uniform(-2, 1)
    ...
    for p, value in pset.items():
        if p not in results[0]:
            results[0][p] = runner[p] # Preserve original 4-stable pset
        runner[p] = value
